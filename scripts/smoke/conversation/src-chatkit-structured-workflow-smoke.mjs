...
const workflowResult = await mod.runStructuredChatKitWorkflow(
  {
    userContextId: "workflow-user",
    conversationId: "workflow-thread",
    missionRunId: "workflow-run",
  ...
  },
  {
    executeStep: async ({ prompt }) => {
      ...
      const stepLabel = prompt.includes("Current step (research)")
        ? "research"
...
console.log("[src-chatkit-structured-workflow-smoke] PASS");
