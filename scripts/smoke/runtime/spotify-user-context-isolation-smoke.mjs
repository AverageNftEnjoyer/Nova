...
await run("RSI-1 Playback route scopes user context to local identity", async () => {
  const routeSource = read("hud/app/api/integrations/spotify/playback/route.ts")
  assert.equal(routeSource.includes("requireLocalUser"), true)
  assert.equal(routeSource.includes("const { userId } = await requireLocalUser()"), true)
  assert.equal(routeSource.includes("userContextId: userId"), true)
})
...
await run("RSI-4 Runtime Spotify HUD adapter preserves scoped auth headers", async () => {
  const adapterSource = read("src/runtime/modules/services/spotify/provider-adapter/hud-http/index.js")
  ...
  assert.equal(adapterSource.includes("Authorization: `Bearer ${token}`") || adapterSource.includes("headers[sharedHeader]"), true)
  assert.equal(fs.existsSync("src/runtime/modules/services/spotify/provider-adapter/direct-now-playing/index.js"), false)
})
