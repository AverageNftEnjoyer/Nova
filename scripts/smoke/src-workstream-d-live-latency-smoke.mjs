...
const latencyTargetMet = avgLatencyMs <= 9000 && p95LatencyMs <= 18500;
...
const report = {
  ...
  summary: {
    ...
    okTurns: turnMetrics.filter((turn) => turn.ok).length,
    avgLatencyMs,
    p50LatencyMs,
    p95LatencyMs,
    maxLatencyMs,
...
const reportPath = path.join(process.cwd(), ".agent", "logs", `${report.id}-report.json`);
...
console.log(`avg=${avgLatencyMs}ms p50=${p50LatencyMs}ms p95=${p95LatencyMs}ms max=${maxLatencyMs}ms`);
