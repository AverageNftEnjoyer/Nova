...
function selectDataset() {
  ...
  const fixturePath = path.join(root, "scripts", "smoke", "fixtures", "chatkit-release-events.jsonl");
  const minLiveSamples = Number.parseInt(process.env.NOVA_CHATKIT_RELEASE_MIN_LIVE_SAMPLES || "8", 10) || 8;
  const lookbackMinutes = Number.parseInt(process.env.NOVA_CHATKIT_RELEASE_LOOKBACK_MINUTES || "30", 10) || 30;
  const userContextPrefix = String(process.env.NOVA_CHATKIT_RELEASE_USER_CONTEXT_PREFIX || "").trim().toLowerCase();
...
const root = process.cwd();
const baselinePath = path.join(root, "scripts", "smoke", "quality", "chatkit-release-baseline.json");
const baseline = loadBaseline(baselinePath);
const dataset = selectDataset();
const metrics = computeMetrics(dataset.events);

assert.equal(metrics.counts.totalEvents > 0, true, "ChatKit release-readiness gate requires non-empty event data");

const evaluation = evaluateGate(metrics, baseline);
const report = {
  ts: new Date().toISOString(),
  suite: "chatkit-release-readiness",
...
const reportPath = path.join(root, "archive", "logs", "chatkit-release-readiness-report.json");
fs.mkdirSync(path.dirname(reportPath), { recursive: true });
fs.writeFileSync(reportPath, JSON.stringify(report, null, 2), "utf8");

if (!evaluation.pass) {
  throw new Error(
    `[src-chatkit-release-readiness] BLOCKED checks=${JSON.stringify(evaluation.checks)} report=${reportPath}`,
  );
}

console.log(`[src-chatkit-release-readiness] PASS mode=${dataset.mode} report=${reportPath}`);
