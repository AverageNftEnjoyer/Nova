...
const { withTimeout } = providersRuntimeCompatModule;
...
await run("WSD7 timeout wrapper aborts cooperative work on deadline", async () => {
  let ticks = 0;
  const startMs = Date.now();
  const timedTask = ({ signal }) =>
  ...
  await assert.rejects(
    withTimeout(timedTask, 30, "cooperative-task"),
    /(timed out after 30ms|aborted)/i,
  );
  const elapsedMs = Date.now() - startMs;
  const ticksAtTimeout = ticks;
  await new Promise((resolve) => setTimeout(resolve, 60));
  assert.equal(ticks, ticksAtTimeout, "cooperative task should stop ticking after timeout");
  assert.equal(elapsedMs < 170, true, `expected early timeout path, got ${elapsedMs}ms`);
});
