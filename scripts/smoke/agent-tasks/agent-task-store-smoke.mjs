...
const { listAgentTaskBudgetEvents } = await import(pathToFileURL(path.join(repoRoot, "src", "db", "agent-task-budget-events.js")).href)
...
// Budgets are on cost by default: $2.00, no token limit.
assert.deepEqual(fresh.budget, { costUsd: 2, tokens: null, costSource: "default", tokenSource: "none", active: true })
...
// Budget pause: spent $2.10 of the $2.00 default (the 500k tokens do not count: no token limit).
db.prepare(`UPDATE agent_tasks SET status = 'paused', pause_reason = 'budget', budget_state = 'exhausted', cost_usd = 2.10,
  tokens_in = 500000, tokens_out = 1000, error = 'Paused: budget' WHERE id = ?`).run(fresh.id)
...
await assert.rejects(store.raiseTaskBudget("store-smoke", fresh.id, { costBudgetUsd: 2.1 }), /above the \$2.1 this task already spent/)
...
assert.deepEqual(listAgentTaskBudgetEvents("store-smoke", { taskId: fresh.id }), [], "a refused raise records nothing")
const raised = await store.raiseTaskBudget("store-smoke", fresh.id, { costBudgetUsd: 2.5 })
assert.equal(raised.status, "queued")
assert.equal(raised.costBudgetUsd, 2.5)
assert.equal(raised.budgetState, "warning", "2.10/2.50 >= 80%")
// The raise lands in the budget history.
const raisedEvents = listAgentTaskBudgetEvents("store-smoke", { taskId: fresh.id })
assert.equal(raisedEvents.length, 1)
assert.deepEqual(
  [raisedEvents[0].kind, raisedEvents[0].state, raisedEvents[0].costBudgetUsd, raisedEvents[0].tokenBudget, raisedEvents[0].spentUsd],
  ["raised", "warning", 2.5, null, 2.1],
)
...
assert.deepEqual({ ...row }, { status: "queued", cost_budget_usd: 2.5, budget_state: "warning", pause_reason: null })
...
const saved = store.updateTaskBudgetSettings("store-smoke", { defaultCostBudgetUsd: 3, defaultTokenBudget: null })
assert.equal(saved.defaultCostBudgetUsd, 3)
...
assert.deepEqual(resumed.budget, { costUsd: 3, tokens: null, costSource: "default", tokenSource: "none", active: true })

// A saved token default is kept and applies next to the cost default.
const withTokens = store.updateTaskBudgetSettings("store-smoke", { defaultTokenBudget: 400000 })
assert.deepEqual([withTokens.defaultCostBudgetUsd, withTokens.defaultTokenBudget], [3, 400000])
assert.equal((await store.getTask("store-smoke", fresh.id)).budget.tokens, 400000)
store.updateTaskBudgetSettings("store-smoke", { defaultTokenBudget: null })

// Deleting a task removes its budget history.
assert.equal(await store.deleteTask("store-smoke", fresh.id), true)
assert.deepEqual(listAgentTaskBudgetEvents("store-smoke", { taskId: fresh.id }), [])

// A retired model ID is stored as its current replacement (retired-model-aliases); a current ID is unchanged.
const retiredModelTask = await store.createTask("store-smoke", { prompt: "Retired model", agent: "claude", model: "claude-sonnet-4-20250514" })
assert.equal(retiredModelTask.model, "claude-sonnet-5")
assert.equal(fresh.model, "gpt-5.6-terra")
