...
await run("Flag off keeps Coinbase unavailable with safe messaging", async () => {
  ...
  assert.ok(spot);
  const payload = JSON.parse(await spot.execute({ userContextId: "u-rollout-off", symbolPair: "BTC-USD" }));
...
await run("Alpha and beta cohort gates enforce userContextId targeting", async () => {
  setEnv("NOVA_COINBASE_ROLLOUT_KILL_SWITCH", "0");
...
await run("Ramp percentage gate is deterministic", async () => {
  setEnv("NOVA_COINBASE_ROLLOUT_STAGE", "ramp");
  setEnv("NOVA_COINBASE_ROLLOUT_SALT", "rollout-fixed-salt");
...
await run("Rollback kill switch disables both fast-path and tools deterministically", async () => {
...
await run("Telemetry health thresholds enforce go no-go", async () => {
  resetCoinbaseObservabilityForTests();
...
await run("Rollout report includes mission success KPI numerator denominator", async () => {
  resetCoinbaseObservabilityForTests();
  recordCoinbaseMetric({
    endpoint: "tool.coinbase_portfolio_report",
    ok: true,
...
const reportPath = path.join(process.cwd(), "archive", "logs", "coinbase-rollout-controls-report.json");
