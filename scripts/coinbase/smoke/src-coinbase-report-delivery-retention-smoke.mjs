...
await run("P8-D2 notification dispatcher includes email adapter", async () => {
  const source = read("hud/lib/notifications/dispatcher/index.ts");
...
await run("P8-R3 retention pruning is user-scoped and does not leak", async () => {
  const dbPath = path.join(process.cwd(), "scripts", "coinbase", ".tmp", "coinbase-report-delivery-retention-smoke.sqlite");
...
const reportPath = path.join(process.cwd(), "archive", "logs", "coinbase-report-delivery-retention-smoke-report.json");
