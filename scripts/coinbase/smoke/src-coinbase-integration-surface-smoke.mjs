...
await run("P11-I1 OAuth connect/refresh/revoke integration tests are user-scoped", async () => {
  ...
  const store = new CoinbaseDataStore(tempDb("coinbase-integration-surface-oauth.sqlite"));
  try {
...
await run("P11-CI1 CI smoke command wiring includes Coinbase enabled flows", async () => {
  ...
  const required = [
    ...
    "smoke:src-coinbase-mission-runtime",
    "smoke:src-coinbase-command-matrix",
    "smoke:src-coinbase-report-delivery-retention",
    "smoke:src-coinbase-observability-resilience",
    "smoke:src-coinbase-privacy-consent",
    "smoke:src-coinbase-integration-surface",
  ];
...
const reportPath = path.join(process.cwd(), "archive", "logs", "coinbase-integration-surface-smoke-report.json");
