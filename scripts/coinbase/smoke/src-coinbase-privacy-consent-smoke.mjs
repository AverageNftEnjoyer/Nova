...
await run("P10-C1 transaction access is consent-gated in tool and export API paths", async () => {
  const toolSource = read("src/tools/builtin/coinbase-tools/index.ts");
...
await run("P10-P1 privacy controls persist and retention remains integrated", async () => {
  const dbPath = path.join(process.cwd(), "scripts", "coinbase", ".tmp", "coinbase-privacy-consent-smoke.sqlite");
...
await run("P10-S1 secure delete purges scoped sensitive Coinbase data and preserves user isolation", async () => {
  const dbPath = path.join(process.cwd(), "scripts", "coinbase", ".tmp", "coinbase-privacy-consent-delete-smoke.sqlite");
  fs.rmSync(dbPath, { force: true });
  const store = new CoinbaseDataStore(dbPath);
  try {
...
const reportPath = path.join(process.cwd(), "archive", "logs", "coinbase-privacy-consent-smoke-report.json");
