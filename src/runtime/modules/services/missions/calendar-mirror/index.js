...
import { resolveTimezone as resolveTimezoneShared } from "../../shared/timezone/index.js";
...
function requireFunction(dependencies, key) {
  ...
  if (typeof candidate !== "function") {
    throw new Error(`Mission calendar mirror dependency "${key}" is required.`);
  }
  return candidate;
}

function hashEventId(seed, prefix) {
...
function localWeekdayFromDayStamp(dayStamp, timezone, dependencies) {
  const toIsoInTimezone = requireFunction(dependencies, "toIsoInTimezone");
  const getLocalParts = requireFunction(dependencies, "getLocalParts");
...
function nextStartAtFromDailyLikeSchedule(params, dependencies) {
  ...
  for (let offset = 0; offset <= maxDaysAhead; offset += 1) {
    ...
    if (allowedDays.size > 0) {
      const weekday = localWeekdayFromDayStamp(candidateDay, timezone, dependencies);
...
function buildMissionRecurrence(trigger, timezone) {
...
function toHumanSummary(value, fallback) {
...
function formatMissionCadence(params, dependencies) {
  const resolveTimezone = dependencies?.resolveTimezone || resolveTimezoneShared;
...
async function resolveMirrorAccount(scope, dependencies) {
...
export async function syncMissionScheduleToGoogleCalendar(params = {}, dependencies = {}) {
  const createCalendarEvent = requireFunction(dependencies, "createCalendarEvent");
  const deleteCalendarEvent = requireFunction(dependencies, "deleteCalendarEvent");
  const estimateDurationMs = requireFunction(dependencies, "estimateDurationMs");
  const warn = dependencies?.warn || (() => {});
  const info = dependencies?.info || (() => {});
  const resolveTimezone = dependencies?.resolveTimezone || resolveTimezoneShared;
  ...
  const startAt = mode === "interval"
    ...
    : nextStartAtFromDailyLikeSchedule({ now, timezone, time, days, includePastToday: false }, dependencies);
  const durationMs = Math.max(15 * 60 * 1000, estimateDurationMs(Array.isArray(mission.nodes) ? mission.nodes.length : 1));
  ...
  const description = truncateText([humanSummary, `Schedule: ${cadence}.`].filter(Boolean).join("\n"), MAX_EVENT_DESCRIPTION_CHARS);
...
export async function removeMissionScheduleFromGoogleCalendar(params = {}, dependencies = {}) {
  ...
  let mirror;
  try {
    mirror = await resolveMirrorAccount(params.scope, dependencies);
  } catch {
    return;
  }
