...
export function resolveConversationTitle(params: {
  ...
  const { title } = deriveConversationTitle(params.messages, params.now)
  const baseTitle = normalizeWhitespace(title) || DEFAULT_CONVERSATION_TITLE
...
export function autoTitle(messages: ChatMessage[]): string {
  return deriveConversationTitle(messages).title
}
