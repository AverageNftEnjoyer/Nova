...
export function buildIntegrationCatalog(config: IntegrationsConfig): IntegrationCatalogItem[] {
  const items: IntegrationCatalogItem[] = [
    ...
    },
    {
      ...
      label: "Slack",
      kind: "channel",
      connected: config.slack.connected && config.slack.webhookUrl.trim().length > 0,
      source: "core",
      capabilities: ["output"],
      updatedAt: config.updatedAt,
    },
    ...
    },
    {
      ...
      label: "YouTube",
      kind: "api",
      connected: config.youtube.connected && (config.youtube.refreshTokenEnc.trim().length > 0 || config.youtube.accessTokenEnc.trim().length > 0),
      source: "core",
      capabilities: ["fetch"],
      updatedAt: config.updatedAt,
    },
