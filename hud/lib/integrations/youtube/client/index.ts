...
import { fromYouTubeHttpStatus, isTransientHttpStatus, youtubeError } from "../errors/index"
...
export async function youtubeFetchWithRetry(
  input: string,
  init: RequestInit,
  options: YouTubeRequestOptions,
): Promise<Response> {
  const operation = String(options.operation || "youtube_request")
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    try {
      ...
      if (aborted && options.signal?.aborted) {
        throw youtubeError("youtube.cancelled", `${operation} cancelled.`, { status: 499, cause: error })
      }
      ...
      if (timeoutLike) {
        ...
        throw youtubeError("youtube.timeout", `${operation} timed out.`, {
          status: 504,
          retryable: true,
          cause: error,
        })
      ...
      throw youtubeError("youtube.network", `${operation} network failure.`, {
        status: 503,
        retryable: true,
        cause: error,
      })
  ...
  throw youtubeError("youtube.transient", `${operation} exhausted retries.`, { status: 503, retryable: true })
}

export async function readYouTubeErrorMessage(response: Response, fallback: string): Promise<string> {
  const payload = await response.json().catch(() => null)
  if (payload && typeof payload === "object") {
    const record = payload as {
      error?: { message?: string; errors?: Array<{ reason?: string; message?: string }> } | string
      error_description?: string
      message?: string
    }
    const nestedError = record.error && typeof record.error === "object"
      ? String(record.error.message || record.error.errors?.[0]?.message || record.error.errors?.[0]?.reason || "").trim()
      : ""
...
export async function assertYouTubeOk(response: Response, fallback: string): Promise<void> {
  if (response.ok) return
  const message = await readYouTubeErrorMessage(response, fallback)
  throw fromYouTubeHttpStatus(response.status, message)
}