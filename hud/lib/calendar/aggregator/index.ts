...
export async function aggregateCalendarEvents(
  ...
  const integrationsConfig = await loadIntegrationsConfig(integrationScope ?? { userId }).catch((err) => {
    console.warn("[aggregator] Failed to load integrations config:", err instanceof Error ? err.message : String(err))
    return null
  })
