 ...
 * the Mission graph so Builder edits and calendar edits don't conflict.
 *
 * Stored at: .agent/user-context/<userId>/calendar-overrides.json
...
export interface RescheduleRecord {
  missionId: string
  userId: string
  originalTime: string       // ISO8601 — the time before override
  overriddenTime: string     // ISO8601 — the new calendar-set time
...
function resolveOverridesFile(userId: string): string {
  return path.join(resolveUserContextRoot(), userId, "calendar-overrides.json")
}
...
async function atomicWriteJson(filePath: string, payload: unknown): Promise<void> {
  ...
  const next = previous
    ...
    .then(async () => {
      ...
      try { await copyFile(resolved, `${resolved}.bak`) } catch { /* first write */ }
...
// ─── Per-user lock for read-modify-write ──────────────────────────────────
...
export async function loadRescheduleOverrides(userId: string): Promise<RescheduleRecord[]> {
  ...
  try {
    ...
    return parsed
      ...
      .filter((r): r is RescheduleRecord => r !== null)
  } catch {
    return []
  }
}
...
export async function getRescheduleOverride(
  ...
  return overrides.find((r) => r.missionId === missionId) ?? null
}
...
export async function setRescheduleOverride(
  ...
  const next = prev.catch(() => undefined).then(async () => {
    ...
    const existing = overrides.find((r) => r.missionId === missionId)
    if (existing) {
...
export async function deleteRescheduleOverride(
  ...
  const next = prev.catch(() => undefined).then(async () => {
    ...
    const after = overrides.filter((r) => r.missionId !== missionId)
