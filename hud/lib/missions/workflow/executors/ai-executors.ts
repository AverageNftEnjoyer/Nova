...
export async function executeAiSummarize(
  ...
  try {
    const override = node.integration || node.model
      ...
      : undefined
    const result = await completeWithConfiguredLlm(system || "", fullPrompt, 2200, ctx.scope, override, { refId: ctx.runId, callSite: "mission.ai-summarize" })
...
export async function executeAiClassify(
  ...
  try {
    const override = node.integration || node.model
      ? { provider: node.integration || undefined, model: node.model || undefined }
      : undefined
    const result = await completeWithConfiguredLlm("", truncateForModel(prompt, MAX_PROMPT_CHARS), 500, ctx.scope, override, { refId: ctx.runId, callSite: "mission.ai-classify" })
...
export async function executeAiExtract(
  ...
  try {
    const override = node.integration || node.model
      ? { provider: node.integration || undefined, model: node.model || undefined }
      : undefined
    const result = await completeWithConfiguredLlm("", truncateForModel(prompt, MAX_PROMPT_CHARS), 2200, ctx.scope, override, { refId: ctx.runId, callSite: "mission.ai-extract" })
...
export async function executeAiGenerate(
  ...
  try {
    const override = node.integration || node.model
      ? { provider: node.integration || undefined, model: node.model || undefined }
      : undefined
    const result = await completeWithConfiguredLlm(node.systemPrompt || "", fullPrompt, 2200, ctx.scope, override, { refId: ctx.runId, callSite: "mission.ai-generate" })
    return { ok: true, text: result.text, data: { text: result.text, provider: result.provider, model: result.model } }
  } catch (err) {
    return { ok: false, error: String(err) }
  }
...
export async function executeAiChat(
  ...
  try {
    const override = node.integration || node.model
      ? { provider: node.integration || undefined, model: node.model || undefined }
      : undefined
    const result = await completeWithConfiguredLlm(system || "", truncateForModel(combinedPrompt, MAX_PROMPT_CHARS), 2200, ctx.scope, override, { refId: ctx.runId, callSite: "mission.ai-chat" })
