 ...
 * Version History:
 *
 * - V.72 Alpha (2026-09-24): Token efficiency: stable prompt prefix + prompt caching + tool output caps
 ...
 *     - Tool output caps: one registry (`src/tools/core/output-caps`) applied by the tool executor to every result, so no tool can put an unbounded blob into the tool loop. `read` returns at most 400 lines / 32,000 chars per call when no range is given (it was uncapped); `memory_get` pages at 12,000 chars (was one 32,000 cut) and `web_fetch` at 16,000, both with a new `offset` parameter; `grep` gets a 12,000-char cap, `ls` 8,000, and any other tool a 64,000 safety net. Existing caps (web_search 6,000, memory_search 8,000, exec 8,000, browser_agent 12,000/32,000) moved into the registry unchanged. Every marker now says how much was shown and cut and names the exact next call (for example `call read with {"path": ..., "startLine": 401, "endLine": 800}`) instead of `... [truncated]`. Offline, an agent task that reads a 3,000-line log uses about 78% fewer input tokens.
 *     - The Claude tool loop now wraps web_search/web_fetch results as untrusted external content and flags suspicious patterns, like the OpenAI-compatible loop.
 ...
 *     - Tests: `smoke:src-tools` pages huge `read` and `memory_get` results by following their markers and checks the pages rebuild the source exactly, and caps huge `exec` and unregistered-tool output; `smoke:token-baseline` adds a large-file agent task. `smoke:token-baseline` also checks the static prefix is identical on every call, the per-turn block reaches the model, Claude requests carry the breakpoints, simulated provider caches hit on every call after the first, and cache reads reach `llm_usage` and the session totals; `smoke:src-prompt` checks per-turn context survives a fresh-install persona.
 *
