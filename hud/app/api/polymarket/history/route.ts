...
function normalizeRange(value: string | null): PolymarketHistoryRange {
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const range = normalizeRange(url.searchParams.get("range") || url.searchParams.get("interval"))
    if (!tokenId) {
      return NextResponse.json({ ok: false, error: "tokenId is required." }, { status: 400 })
    }
    const points = await fetchPolymarketPriceHistory(tokenId, range)
    return NextResponse.json({ ok: true, tokenId, range, points })
  } catch (error) {
