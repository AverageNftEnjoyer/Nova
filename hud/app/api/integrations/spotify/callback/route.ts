...
function isPopupFlow(returnTo: string): boolean {
  return returnTo.includes("spotifyPopup=1")
}
...
<html lang="en">
  ...
  <body>
    <div class="card">
      <h1>Spotify ${params.status === "success" ? "connected" : "connection failed"}</h1>
...
export async function GET(req: Request) {
  ...
  const parsedState = parseSpotifyOAuthState(stateRaw)
  const returnTo = parsedState?.returnTo || "/integrations"
  const popupFlow = isPopupFlow(returnTo)

  if (!parsedState) {
    const error = spotifyError("spotify.invalid_state", "Invalid Spotify OAuth state.", { status: 400 })
    logSpotifyApi("callback.invalid_state", { code: error.code, message: error.message })
    return popupCloseResponse({
      status: "error",
      message: error.message,
      returnTo: "/integrations?spotifyPopup=1",
    })
  }
  ...
  if (!code) {
    ...
    logSpotifyApi("callback.missing_code", {
  ...
  try {
    logSpotifyApi("callback.exchange.begin", {
      userContextId: parsedState.userId,
      returnTo,
    })
    await exchangeCodeForSpotifyTokens(code, parsedState.codeVerifier, {
      userId: parsedState.userId,
      allowServiceRole: true,
      serviceRoleReason: "spotify-oauth-callback",
    })
    logSpotifyApi("callback.exchange.success", {
      userContextId: parsedState.userId,
      returnTo,
    })
    if (popupFlow) {
      return popupCloseResponse({ status: "success", message: "Spotify connected.", returnTo })
    }
    return NextResponse.redirect(new URL(withStatus(returnTo, "success", "Spotify connected."), requestUrl.origin), { status: 302 })
  } catch (error) {
    const normalized = error instanceof Error ? error.message : "Failed to connect Spotify."
    logSpotifyApi("callback.exchange.failed", {
      userContextId: parsedState.userId,
      returnTo,
      error: normalized,
    })
